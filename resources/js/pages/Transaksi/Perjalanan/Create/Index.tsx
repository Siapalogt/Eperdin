...
export default function Create({ templates, kategoris }: Props) {
    ...
    // Otomatis hitung selisih hari jika tanggal berangkat & pulang diisi
    useEffect(() => {
    ...
    return (
        <AppLayout title="Buat Usulan Perjalanan Dinas">
            <Head title="Buat Perjalanan Dinas Baru" />
            ...
            <div className="max-w-5xl mx-auto py-4">
                ...
                <div className="mb-6">
                    ...
                    <h1 className="text-2xl font-black text-slate-800 tracking-tight">Formulir Inisiasi Perjalanan Dinas</h1>
                    <p className="text-xs text-slate-500 mt-1">
                        Buat usulan surat perjalanan dinas baru. Semua data awal akan disimpan dengan status <strong className="text-slate-700">Draft</strong>.
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {/* KARTU 1: INISIASI & INFORMASI UTAMA */}
                    <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-5">
                        <div className="flex items-center space-x-2 border-b border-slate-100 pb-3">
                            <div className="w-2 h-5 bg-indigo-600 rounded-full"></div>
                            <h2 className="text-xs font-bold text-slate-800 uppercase tracking-wider">1. Inisiasi & Template Utamanya</h2>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            {/* Nomor Perjalanan / SPT */}
                            <div>
                                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                                    Nomor Surat Perjalanan (SPT) <span className="text-rose-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    required
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                                    Pilih Paket Template Biaya <span className="text-rose-500">*</span>
                                </label>
                                <select
                                    required
                                ...
                                {errors.template_perjalanan_id && <span className="text-rose-500 text-xs mt-1 block font-semibold">{errors.template_perjalanan_id}</span>}
                            </div>

                            {/* Nama Kegiatan */}
                            <div className="md:col-span-2">
                                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                                    Nama Kegiatan Dinas <span className="text-rose-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    required
                                    ...
                                    placeholder="Masukkan rincian nama agenda kegiatan dinas..."
                                    className="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-300 bg-slate-50/50 focus:bg-white focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 transition duration-150 font-medium text-slate-800"
                                />
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-5">
                        <div className="flex items-center space-x-2 border-b border-slate-100 pb-3">
                            ...
                            <h2 className="text-xs font-bold text-slate-800 uppercase tracking-wider">2. Tujuan & Waktu Pelaksanaan</h2>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                                    Kota / Daerah Tujuan <span className="text-rose-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    required
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                                    Lokasi Spesifik <span className="text-slate-400 font-normal">(Opsional)</span>
                                </label>
                                <input
                                    type="text"
                                    value={data.lokasi}
                        ...
                        {/* Tanggal Berangkat, Pulang & Total Durasi */}
