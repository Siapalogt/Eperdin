...
export default function ModalBiayaBersama({
    ...
    if (!isOpen || !perjalanan) return null;
    ...
    const handleTambahBiayaBersama = (e: React.FormEvent) => {
        e.preventDefault();
        post(`/perjalanan/${perjalanan.id}/biaya-bersama`, {
            preserveScroll: true,
            onSuccess: () => {
                reset();
    ...
    const handleDeleteBiaya = (biayaId: number) => {
        if (confirm('Hapus rincian biaya bersama ini?')) {
            router.delete(`/perjalanan/${perjalanan.id}/biaya-bersama/${biayaId}`, {
                preserveScroll: true,
            });
        }
    };
    ...
    const totalKeseluruhan = perjalanan.biaya_bersama?.reduce((acc: number, cur: any) => acc + Number(cur.total || 0), 0) || 0;
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <div className="p-5 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                    <div>
                        <h3 className="font-black text-lg text-slate-800">
                            Kelola Rincian Biaya Bersama / Kolektif
                        </h3>
                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                            Dokumen Perjalanan: {perjalanan.nomor} ({perjalanan.nama_kegiatan})
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 overflow-y-auto grid grid-cols-1 lg:grid-cols-3 gap-8 flex-grow">
                    ...
                    <div className="lg:col-span-1 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm h-fit">
                        <h4 className="font-bold text-sm text-slate-800 border-b border-slate-100 pb-3 mb-4">
                            Tambah Item Bersama
                        </h4>

                        <form onSubmit={handleTambahBiayaBersama} className="space-y-4">
                            <div>
                            ...
                            <div className="pt-4 border-t border-slate-100 mt-4 space-y-4">
                                ...
                                >
                                    {processing ? 'Menyimpan Data...' : 'Simpan Biaya Bersama'}
                                </button>
                            </div>
                        </form>
                    </div>

                    {/* KANAN: TABEL HASIL USULAN BIAYA BERSAMA */}
                    <div className="lg:col-span-2 space-y-4">
                        <h4 className="font-bold text-sm text-slate-800">Daftar Biaya Bersama yang Tercatat</h4>
                        ...
                        <div className="border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                            <table className="min-w-full divide-y divide-slate-200 text-xs">
                                ...
                                <tbody className="divide-y divide-slate-100 bg-white">
                                    {perjalanan.biaya_bersama && perjalanan.biaya_bersama.length > 0 ? (
                                        perjalanan.biaya_bersama.map((b: any) => (
                                            <tr key={b.id} className="hover:bg-slate-50/80 transition group">
                                                <td className="px-5 py-3">
                                                    <div className="font-bold text-slate-800 text-sm">
                                                        {b.komponen_biaya?.nama || 'Item Biaya'}
                                                    </div>
                                                    {b.keterangan && (
                                        ...
                                        <tr>
                                            <td colSpan={5} className="px-5 py-10 text-center text-slate-400 italic font-medium">
                                                Belum ada rincian biaya bersama yang ditambahkan.
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
