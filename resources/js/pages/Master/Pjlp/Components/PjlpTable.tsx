...
interface PjlpTableProps {
    listPjlp: PjlpItem[];
    activeEditId: number | null;
    onEdit: (item: PjlpItem) => void;
    onDelete: (id: number) => void;
}

export const PjlpTable: React.FC<PjlpTableProps> = ({
    listPjlp,
    activeEditId,
    onEdit,
    onDelete,
    ...
    const filteredPjlp = listPjlp.filter(
        (item) =>
            ...
            <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                        Total terdaftar: <strong className="text-slate-700">{listPjlp.length}</strong> orang
                    </p>
                </div>
                ...
                <table className="min-w-full divide-y divide-slate-100 text-xs">
                    <thead className="bg-slate-50 text-slate-500 font-bold uppercase tracking-wider">
                        <tr>
                            ...
                            <th className="px-5 py-3.5 text-left">Jabatan & Bagian</th>
                            <th className="px-5 py-3.5 text-left">Nomor HP</th>
                        ...
                        {filteredPjlp.length === 0 ? (
                            <tr>
                                <td colSpan={5} className="px-6 py-12 text-center text-slate-400 italic">
                                    Tidak ada data PJLP yang ditemukan.
                                </td>
                            </tr>
                        ) : (
